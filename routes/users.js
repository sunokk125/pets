...
router.get('/', function(req, res, next) {
  ...
  connection.query('SELECT * FROM post WHERE puid = ' + uId + ' ORDER BY ptime DESC;', function (err, rows, fields) {
    if(!err){
      function setFriend() {
      ...
      function setFriend() {
        return new Promise(function (resolve, reject) {
          connection.query('SELECT * FROM user WHERE uid = ' + uId + ';',function (err, urows) {
            if(!err){
              udata = urows[0];
              connection.query('SELECT * FROM friend WHERE fuidm = ' + uId + ';', function (err, fwrows) {
                if (!err) {
                  follow = fwrows.length;
                  connection.query('SELECT * FROM friend WHERE fuidf = ' + uId + ';', function (err, frows) {
                    if (!err) {
                      follower = frows.length;
                      connection.query('SELECT * FROM friend WHERE fuidf = ' + uId + ' and fuidm = ' + muId + ';', function (err, friendRows) {
                        if(!err){
                          if(friendRows[0]){
                            friend = true;
                            resolve();
...
router.post('/search', function (req, res, next) {
  ...
  if(uEmail){
    connection.query("SELECT * FROM user WHERE uemail like '%" + uEmail + "%';", function (err, rows) {
      if(!err){
        if(rows[0]){
          res.json({search: true, udata: rows});
        }else{
